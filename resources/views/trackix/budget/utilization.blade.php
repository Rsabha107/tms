                        ...
                        <div class="card-header p-4 border-bottom border-300 bg-soft">
                            <div class="row g-3 justify-content-between align-items-center">
                                ...
                                <div class="col-12 col-md">
                                    <h2 class="text-900 mb-0"><span class="uil fs-5 lh-1 uil-calculator text-info px-2"></span>Budget Utilization</h2>
                                </div>
                                    ...
                                    <div class="table-responsive scrollbar mb-3">
                                        <table class="table table-sm table-responsive fs--1 mb-0 overflow-hidden" id="dataList">
                                            <thead class="text-900 thead">
                                                <tr>
                                                    <th class="sort white-space-wrap align-middle ps-2" scope="col" data-sort="name" style="width:10%;">PROJECT</th>
                                                    <th class="sort white-space-wrap align-middle ps-2" scope="col" data-sort="name" style="width:10%;">PROJECT BUD</th>
                                                    <th class="sort white-space-wrap align-middle ps-2" scope="col" data-sort="name" style="width:10%;">TOTAL TASK UTIL</th>
                                                    <th class="sort white-space-wrap align-middle ps-2" scope="col" data-sort="name" style="width:10%;">REMAINING</th>
                                                    <th class="sort white-space-wrap align-middle ps-2" scope="col" data-sort="name" style="width:10%;">TASK</th>
                                                    <th class="sort white-space-wrap align-middle ps-5" scope="col" data-sort="department" style="width:15%;">DEPARTMENT</th>
                                            ...
                                            <tbody class="list" id="order-table-body">
                                                ...
                                                $project_budget = App\Http\Controllers\UtilController::getEventBudgetDetails($item->event_id);
                                                $remaining_budget = $project_budget->eventbudget - $project_budget->task_total_budget;
                                                if ($remaining_budget < 0){ $text_color="text-danger" ; } else { $text_color="text-success" ; } $asg_to_names='' ; foreach($assigntonames as $key1=> $item1){
                                                    $asg_to_names = $asg_to_names.', '. $item1->assigned_to_name;
                                                    }
                                                    $asg_to_names = ltrim($asg_to_names, ',');
                                                    @endphp
                                                    <tr class="hover-actions-trigger btn-reveal-trigger position-static">
                                                        <td class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0"> {{$item->project_name}}</td>
                                                        <td class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0"> {{$project_budget->eventbudget}}</td>
                                                        <td class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0"> {{$project_budget->task_total_budget}}</td>
                                                        <td class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0 {{ $text_color }}"> {{$remaining_budget}}</td>
                                                        <td class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0">{{$item->task_name}}</td>
                                                        <td class="align-middle time white-space-nowrap ps-0 projectName py-4"><a class="fw-bold fs-0 ms-5">{{$item->department_name}}</a></td>
