...
class TaskController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('main.task.all', ['projects' => $projects,
                                      'persons' => $persons,
                                      'statuses' => $statuses,
                                     ]);
    }
    ...
    public function allTaskDt()
    {
        ...
        $taskData = $taskData
            ...
            ->through(
                fn ($task) => [
                    'id' => $task->id,
                    'event_id' => $task->project->name,
                    'task_name' => '<div class="align-middle time white-space-wrap ps-2 projectName py-4 fw-bold fs-0"><a href="javascript:void(0)">'. $task->name .'</a></div>',
                    ...
                    'attributes' => '<div class="ms-3 text-secondary">'.(($task->files->count()) ? '<span class="fas fa-file-alt me-1"></span>':"").' '.(($task->notes->count()) ? '<span class="fas fa-paperclip me-1"></span>':"").'</div>',
                    'status' => ($task->status->name == 'Completed') ? '<span class="badge badge-phoenix fs--2 badge-phoenix-success "><span class="badge-label" data-bs-toggle="modal" data-bs-target="#taskStatusModal" id="editTaskStatus" data-id="{{ $item->id }}">' . $task->status->name . '</span><span class="ms-1" data-feather="x" style="height:12.8px;width:12.8px;"></span></span>' : '<span class="badge badge-phoenix fs--2 badge-phoenix-primary "><span class="badge-label" data-bs-toggle="modal" data-bs-target="#taskStatusModal" id="editTaskStatus" data-id="{{ $item->id }}">' . $task->status->name . '</span><span class="ms-1" data-feather="x" style="height:12.8px;width:12.8px;"></span></span>',
