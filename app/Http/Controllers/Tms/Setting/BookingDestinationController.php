...
class BookingDestinationController extends Controller
{
    //
    public function index()
    {
        $statuses = Destination::all();
        return view('tms.setting.destination.list', compact('statuses'));
    }

    public function get($id)
    {
        $op = Destination::findOrFail($id);
    ...
    public function update(Request $request)
    {
        ...
        $status = Destination::findOrFail($request->id);

        // dd($status);

        if ($status->update($formFields)) {
            return response()->json(['error' => false, 'message' => 'Destination updated successfully.']);
        } else {
            return response()->json(['error' => true, 'message' => 'Destination couldn\'t be updated.']);
        }
    }
    ...
    public function list()
    {
        ...
        $ops = Destination::orderBy($sort, $order);
        ...
        $ops = $ops->paginate(request("limit"))->through(function ($op) {
            ...
            $update_action =
                ...
                ' data-table="destination_table" data-bs-toggle="tooltip" data-bs-placement="right" title="Update">' .
                '<i class="fa-solid fa-pen-to-square text-primary"></i></a>';
            $delete_action =
                '<a href="javascript:void(0)" class="btn btn-sm" data-table="destination_table" data-id="' .
                $op->id .
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $error = false;
            $message = 'Destination created succesfully.' . $op->id;
