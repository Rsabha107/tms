...
class ZoneController extends Controller
{
    ...
    public function index()
    {
        ...
        return view('mds.setting.zone.list', compact('venues', 'zones'));
    }

    public function get($id)
    {
        $venue = DeliveryZone::findOrFail($id);
    ...
    public function update(Request $request)
    {
        ...
        $venue = DeliveryZone::findOrFail($request->id);

        // dd($venue);

        if ($venue->update($formFields)) {
            return response()->json(['error' => false, 'message' => 'Zone updated successfully.', 'id' => $venue->id]);
        } else {
    ...
    public function list()
    {
        ...
        $venue = DeliveryZone::orderBy($sort, $order);
        ...
        // $location = Location::find($venue->location_id);
            ...
            return  [
                ...
                'venue' => '<div class="align-middle white-space-wrap fw-bold fs-8 ps-3">' . $venue->venue?->title . '</div>',
    ...
    public function store(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $error = false;
            $message = 'Zone created succesfully.' . $venue->id;
