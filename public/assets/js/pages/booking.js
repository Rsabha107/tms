...
$(document).ready(function () {
    ...
    $("body").on("click", "#show_shcedule_times_modal", function () {
        ...
        $.ajax({
            url: "/mds/booking/get_times/" + booking_date1 + "/" + venue_id,
            type: "get",
            headers: {
...
$("body").on("click", "#booking-note-tab", function (event) {
    ...
    $.ajax({
        url: "/mds/booking/mv/notes/" + tab_value,
        method: "GET",
        async: true,
        success: function (response) {
            g_response = response.view;
...
$("body").on("click", ".removeNoteDiv", function (e) {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            $.ajax({
                url: "/mds/booking/note/delete/" + id,
                type: "DELETE",
                headers: {
                    "X-CSRF-TOKEN": $('meta[name="csrf-token"]').attr(
                        "content"
