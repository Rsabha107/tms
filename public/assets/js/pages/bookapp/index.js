    ...
    var calendar = new FullCalendar.Calendar(calendarEl, {
        ...
        eventDidMount: function (info) {
            ...
            if (info.event.extendedProps.textColor) {
              ...
              info.el.style.textColor = info.event.extendedProps.textColor;
            }
          },
        ...
        dateClick: function (info) {
            ...
            let team_id = $("#bookapp_team_id").val();
            let destination_id = $("#bookapp_destination_id").val();
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    console.log(
                        "response length: " + response.ops.length
                    );
                    let isExists = response.ops.length;
                    // var len = response.length;

                    let html = '<ul class="list-group mb-5" id="myTab" role="tablist">';
                    ...
                    $.each(response.ops, function (key, value) {
                        ...
                        html += '<li class="list-group-item">'+
                                    '<input class="form-check-input me-1" type="radio" name="time_slot_id"'+
                                        'value="'+value.id+'" '+
                                        'id="timeSlotsRadio'+value.id+'" required>'+
                                    '<label class="form-check-label"'+
                                        'for="timeSlotsRadio'+value.id+'">'+value.booking_slot+'</label>'+
                                '</li>';

                    });
                    html += '<div class="invalid-feedback">Please select one or multiple</div>';
                    html += '</ul>';

                    $('#bookapp_form')[0].reset();
                    $('#bookapp_form').removeClass('was-validated');
                    ...
                    if (isExists > 0 && team_id && destination_id) {
                        $("#div3_title").html('<h5 class="mb-4" id="div3_title">Available time slots for '+info.dateStr+'</h5>');
                        $('#submit_booking').prop('disabled', false);
                    }
                    else {
                        $("#div3_title").html('<h3 class="mb-4" id="div3_title">Select a date to see available times</h3>');
